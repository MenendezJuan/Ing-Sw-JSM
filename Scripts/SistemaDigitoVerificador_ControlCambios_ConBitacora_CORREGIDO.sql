...
-- ==========================================
-- 4. TRIGGERS PARA HISTORIAL AUTOMÁTICO (CORREGIDOS - SIN DUPLICADOS)
-- ==========================================
...
BEGIN
    ...
    BEGIN
        ...
        -- REGISTRAR EN BITÁCORA LA OPERACIÓN (UNA SOLA VEZ)
        DECLARE @ProductoIds NVARCHAR(MAX) = '';
        SELECT @ProductoIds = @ProductoIds + CAST(Id AS NVARCHAR) + ',' FROM inserted;
        SET @ProductoIds = LEFT(@ProductoIds, LEN(@ProductoIds) - 1); -- Quitar última coma
        
        DECLARE @Descripcion NVARCHAR(MAX) = 'Operación ' + @TipoOperacion + ' en Producto(s) ID: ' + @ProductoIds + '. DV actualizado automáticamente.';
    ...
    BEGIN
        ...
        DECLARE @ProductoEliminadoIds NVARCHAR(MAX) = '';
        SELECT @ProductoEliminadoIds = @ProductoEliminadoIds + CAST(Id AS NVARCHAR) + ',' FROM deleted;
        SET @ProductoEliminadoIds = LEFT(@ProductoEliminadoIds, LEN(@ProductoEliminadoIds) - 1);
        
        DECLARE @DescripcionDelete NVARCHAR(MAX) = 'Producto(s) eliminado(s) ID: ' + @ProductoEliminadoIds + '. Registro movido a historial.';
        INSERT INTO Bitacora (Fecha, Accion, Usuario, Descripcion)
...
BEGIN
    ...
    BEGIN
        ...
        -- REGISTRAR EN BITÁCORA LA OPERACIÓN (UNA SOLA VEZ)
        DECLARE @VentaIds NVARCHAR(MAX) = '';
        SELECT @VentaIds = @VentaIds + CAST(Id AS NVARCHAR) + ',' FROM inserted;
        SET @VentaIds = LEFT(@VentaIds, LEN(@VentaIds) - 1);
        
        DECLARE @DescripcionVenta NVARCHAR(MAX) = 'Operación ' + @TipoOperacion + ' en Venta(s) ID: ' + @VentaIds + '. DV actualizado automáticamente.';
        INSERT INTO Bitacora (Fecha, Accion, Usuario, Descripcion)
    ...
    BEGIN
        ...
        -- REGISTRAR ELIMINACIÓN EN BITÁCORA
        DECLARE @VentaEliminadaIds NVARCHAR(MAX) = '';
        SELECT @VentaEliminadaIds = @VentaEliminadaIds + CAST(Id AS NVARCHAR) + ',' FROM deleted;
        SET @VentaEliminadaIds = LEFT(@VentaEliminadaIds, LEN(@VentaEliminadaIds) - 1);
        
        DECLARE @DescripcionDeleteVenta NVARCHAR(MAX) = 'Venta(s) eliminada(s) ID: ' + @VentaEliminadaIds + '. Registro movido a historial.';
        INSERT INTO Bitacora (Fecha, Accion, Usuario, Descripcion)
...
-- ==========================================
-- 6. STORED PROCEDURE PARA VERIFICACIÓN MASIVA CORREGIDO
-- ==========================================

-- SP para verificar integridad masiva con registro en bitácora (CORREGIDO)
...
BEGIN
    ...
    -- Obtener conteo según tipo de entidad
    IF @TipoEntidad = 'Usuario'
    ...
    ELSE
    BEGIN
        RAISERROR('Tipo de entidad no soportado: %s', 16, 1, @TipoEntidad);
        RETURN;
    END
    ...
    -- Por simplicidad, asumimos que no hay inconsistencias en esta implementación inicial
    SET @InconsistenciasEncontradas = 0;
...
-- ==========================================
-- 7. CALCULAR DÍGITOS VERIFICADORES EXISTENTES (SIMPLIFICADO)
-- ==========================================
...
PRINT '✅ Triggers automáticos para historial (SIN DUPLICADOS)';
...
PRINT '';
PRINT 'NOTA: Los triggers han sido corregidos para evitar ejecuciones duplicadas.';
PRINT '';
